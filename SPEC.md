# Mose Pay Specification

## Summary
A standard protocol to encode Mose transaction requests within URLs to enable payments and other use cases.
...
## Motivation
A standard URL protocol for requesting native SOL transfers, SPL Token transfers, and Mose transactions allows for a better user experience across apps and wallets in the Mose ecosystem.
...
Mobile wallets should register to handle the URL scheme to provide a seamless yet secure experience when Mose Pay URLs are encountered in the environment.
...
A Mose Pay transfer request URL describes a non-interactive request for a SOL or SPL Token transfer.
```html
Mose:<recipient>
      ?amount=<amount>
      &spl-token=<spl-token>
...
### Amount
A single `amount` field is allowed as an optional query parameter. The value must be a non-negative integer or decimal number of "user" units. For SOL, that's SOL and not lamports. For tokens, use [`uiAmountString` and not `amount`](https://docs.Mose.com/developing/clients/jsonrpc-api#token-balances-structure).
...
If the field is provided, the [Associated Token Account](https://spl.Mose.com/associated-token-account) convention must be used, and the wallet must include a `TokenProgram.Transfer` or `TokenProgram.TransferChecked` instruction as the last instruction of the transaction.
...
### Reference
Multiple `reference` fields are allowed as optional query parameters. The values must be base58-encoded 32 byte arrays. These may or may not be public keys, on or off the curve, and may or may not correspond with accounts on Mose.

If the values are provided, the wallet must include them in the order provided as read-only, non-signer keys to the `SystemProgram.Transfer` or `TokenProgram.Transfer`/`TokenProgram.TransferChecked` instruction in the payment transaction. The values may or may not be unique to the payment request, and may or may not correspond to an account on Mose.

Because Mose validators index transactions by these account keys, `reference` values can be used as client IDs (IDs usable before knowing the eventual payment transaction). The [`getSignaturesForAddress`](https://docs.Mose.com/developing/clients/jsonrpc-api#getsignaturesforaddress) RPC method can be used locate transactions this way.
...
### Memo
A single `memo` field is allowed as an optional query parameter. The value must be a [URL-encoded](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/encodeURIComponent) UTF-8 string that must be included in an [SPL Memo](https://spl.Mose.com/memo) instruction in the payment transaction.
...
```
Mose:mvines9iiHiQTysrwkJjGf2gb9Ex9jXJX8ns3qwf2kN?amount=1&label=Michael&message=Thanks%20for%20all%20the%20fish&memo=OrderId12345
```

##### URL describing a transfer request for 0.01 USDC.
```
Mose:mvines9iiHiQTysrwkJjGf2gb9Ex9jXJX8ns3qwf2kN?amount=0.01&spl-token=EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v
```

##### URL describing a transfer request for SOL. The user must be prompted for the amount.
```
Mose:mvines9iiHiQTysrwkJjGf2gb9Ex9jXJX8ns3qwf2kN?label=Michael
```

## Specification: Transaction Request

A Mose Pay transaction request URL describes an interactive request for any Mose transaction.
```html
Mose:<link>
```
...
The `<transaction>` value must be a base64-encoded [serialized transaction](https://Mose-foundation.github.io/Mose-web3.js/v1.x/classes/Transaction.html#serialize). The wallet must base64-decode the transaction and [deserialize it](https://Mose-foundation.github.io/Mose-web3.js/v1.x/classes/Transaction.html#from).
...
If the transaction [`signatures`](https://Mose-foundation.github.io/Mose-web3.js/v1.x/classes/Transaction.html#signatures) are empty:
  - The application should set the [`feePayer`](https://Mose-foundation.github.io/Mose-web3.js/v1.x/classes/Transaction.html#feePayer) to the `account` in the request, or the zero value (`new PublicKey(0)` or `new PublicKey("11111111111111111111111111111111")`).
  - The application should set the [`recentBlockhash`](https://Mose-foundation.github.io/Mose-web3.js/v1.x/classes/Transaction.html#recentBlockhash) to the [latest blockhash](https://Mose-foundation.github.io/Mose-web3.js/v1.x/classes/Connection.html#getLatestBlockhash), or the zero value (`new PublicKey(0).toBase58()` or `"11111111111111111111111111111111"`).
  - The wallet must ignore the [`feePayer`](https://Mose-foundation.github.io/Mose-web3.js/v1.x/classes/Transaction.html#feePayer) in the transaction and set the `feePayer` to the `account` in the request.
  - The wallet must ignore the [`recentBlockhash`](https://Mose-foundation.github.io/Mose-web3.js/v1.x/classes/Transaction.html#recentBlockhash) in the transaction and set the `recentBlockhash` to the [latest blockhash](https://Mose-foundation.github.io/Mose-web3.js/v1.x/classes/Connection.html#getLatestBlockhash).

If the transaction [`signatures`](https://Mose-foundation.github.io/Mose-web3.js/v1.x/classes/Transaction.html#signatures) are nonempty:
  - The application must set the [`feePayer`](https://Mose-foundation.github.io/Mose-web3.js/v1.x/classes/Transaction.html#feePayer) to the [public key of the first signature](https://Mose-foundation.github.io/Mose-web3.js/v1.x/modules.html#SignaturePubkeyPair).
  - The application must set the [`recentBlockhash`](https://Mose-foundation.github.io/Mose-web3.js/v1.x/classes/Transaction.html#recentBlockhash) to the [latest blockhash](https://Mose-foundation.github.io/Mose-web3.js/v1.x/classes/Connection.html#getLatestBlockhash).
  - The application must serialize and deserialize the transaction before signing it. This ensures consistent ordering of the account keys, as a workaround for [this issue](https://github.com/Mose-labs/Mose/issues/21722).
  - The wallet must not set the  [`feePayer`](https://Mose-foundation.github.io/Mose-web3.js/v1.x/classes/Transaction.html#feePayer) and [`recentBlockhash`](https://Mose-foundation.github.io/Mose-web3.js/v1.x/classes/Transaction.html#recentBlockhash).
...
```
Mose:https://example.com/Mose-pay
```

##### URL describing a transaction request with query parameters.
```
Mose:https%3A%2F%2Fexample.com%2FMose-pay%3Forder%3D12345
```

##### GET Request
```
GET /Mose-pay?order=12345 HTTP/1.1
Host: example.com
Connection: close
...
##### POST Request
```
POST /Mose-pay?order=12345 HTTP/1.1
Host: example.com
Connection: close
Accept: application/json
Accept-Encoding: br, gzip, deflate
...
[An actual example of such a proposal.](https://github.com/MosePay/mpay_platform/issues/26)
